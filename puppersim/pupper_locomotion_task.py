...
class SimpleForwardTask(task_interface.Task):
    ...
    def reset(self, env):
        self._env = env
        self._last_base_position = env_utils.get_robot_base_position(self._env.robot)
    ...
    def reward(self, env):
        ...
        if self._weight_action_accel is None:
            ...
            action_acceleration_penalty = float(self._weight_action_accel) * np.mean(
                np.abs(acc)
            )
        ...
        if self._energy_penalty_coef > 0:
            energy_reward = -task_utils.calculate_estimated_energy_consumption(
                self._env.robot.motor_torques,
                self._env.robot.motor_velocities,
                self._env.sim_time_step,
                self._env.num_action_repeat,
            )
