...
class ReacherEnv(gym.Env):
    def __init__(self, max_episode_steps=50, use_success_bonus=False, run_on_robot=False, render=False):
        ...
        self._use_success_bonus = use_success_bonus

        # pick new goal after max_episode_steps steps
    ...
    def reset(self):
        ...
        target_angles = np.random.uniform(-0.1 * math.pi, 0.1 * math.pi, 3)
    ...
    def step(self, actions):
        ...
        reward_dist = -10. * np.linalg.norm(self._get_vector_from_end_effector_to_goal())
        reward_ctrl = 0
        success_bonus = float(abs(reward_dist) < 0.005 and self._use_success_bonus)
        reward = reward_dist + reward_ctrl + success_bonus
        self._cur_steps += 1
        done = self._cur_steps >= self._max_steps
    ...
    def _forward_kinematics(self, joint_angles):
        ...
        foot_pos = np.array([[self._hip_offset], [x1 + x2], [z1 + z2]])
